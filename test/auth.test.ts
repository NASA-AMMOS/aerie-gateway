...
describe('auth helper function tests', () => {
  ...
  test(
    'test auth group filter only considers groups that have a mapping', () => {
  ...
  test(
    'correct union of all allowed roles for auth groups', () => {
  ...
  test(
    'group -> role mapping that doesnt contain default role throws error', () => {
      ...
      expect(() => validateGroupRoleMappings())
          .toThrowError('No roles within DEFAULT_ROLE list were found in the group to role mapping.');
  });
  ...
  describe(
    'reference example', () => {
      ...
      test(
        'error is throw since group_E doesnt contain a default role',
        () => {
          vi.stubEnv("DEFAULT_ROLE", JSON.stringify(default_roles));
          vi.stubEnv("AUTH_GROUP_ROLE_MAPPINGS", JSON.stringify(group_role_mapping));
      ...
      test(
        'user with only group_C auth group membership',
        () => {
          vi.stubEnv("DEFAULT_ROLE", JSON.stringify(default_roles));
          vi.stubEnv("AUTH_GROUP_ROLE_MAPPINGS", JSON.stringify(group_role_mapping));
      ...
      test(
        'user with group B and D auth group membership',
        () => {
          vi.stubEnv("DEFAULT_ROLE", JSON.stringify(default_roles));
          vi.stubEnv("AUTH_GROUP_ROLE_MAPPINGS", JSON.stringify(group_role_mapping));
